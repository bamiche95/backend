...
import { generateToken, verifyToken } from './jwtUtilities.js';
...
const allowedOrigins = [
      'http://localhost:5173',
      'https://localhost:5173',     // React web
  'http://localhost:8081',          // React Native metro server (dev)
  'null',                          // React Native fetch sometimes has no origin
  'http://192.168.1.100:8081',     // Your backend IP address for React Native
];
...
const httpsOptions = {
  key: fs.readFileSync(path.resolve(__dirname, '../web/certs/localhost-key.pem')),
  cert: fs.readFileSync(path.resolve(__dirname, '../web/certs/localhost.pem')),
};
...
httpServer.listen(5000, '0.0.0.0', () => {
  console.log("Backend HTTP server running on http://0.0.0.0:5000");
});
...
io.on("connection", (socket) => {
  ...
  if (userId) {
    ...
    console.warn(`❌ No user found for socket ${socket.id}`);
  }
...
io.on("connection", (socket) => {
  ...
  if (!userId) {
    console.warn(`❌ Invalid user for socket ${socket.id}`);
    return socket.disconnect(true);
  }

  console.log(`✅ User ${userId} connected via socket ${socket.id}`);

  // Join personal room (e.g., for notifications)
  socket.join(userId.toString());

  // Join custom room for chat
  socket.on('joinRoom', (roomId) => {
    socket.join(roomId);
    console.log(`User ${userId} joined room ${roomId}`);
  });

  socket.on("sendMessage", async ({ to, text, roomId, replyToMessageId = null, productId = null }) => {
    const safeContent = xss(text);

    try {
      await withConnection(async (connection) => {
        const insertQuery = productId
          ? `INSERT INTO nearby_message (room_id, sender_id, recipient_id, text, reply_to_message_id, product_id) VALUES (?, ?, ?, ?, ?, ?)`
          : `INSERT INTO nearby_message (room_id, sender_id, recipient_id, text, reply_to_message_id) VALUES (?, ?, ?, ?, ?)`;

        const params = productId
          ? [roomId, userId, to, safeContent, replyToMessageId, productId]
          : [roomId, userId, to, safeContent, replyToMessageId];
  ...
  socket.on("markMessagesAsRead", async ({ roomId }) => {
    try {
      await withConnection(async (connection) => {
        const query = `
        ...
        `;
        await connection.query(query, [userId, roomId]);
  ...
  socket.on("sendReaction", async ({ messageId, emoji, roomId }) => {
    try {
      await withConnection(async (connection) => {
  ...
  socket.on("removeReaction", async ({ messageId, emoji, roomId }) => {
    try {
      await withConnection(async (connection) => {
        const deleteQuery = `
...
app.get("/api/check-username", authenticateJWT, async (req, res) => {
...
app.get("/api/check-email", authenticateJWT, async (req, res) => {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const loginResult = await withConnection(async (connection) => {
      ...
      // Prepare user object for token (avoid sensitive info)
      const sessionUser = {
        id: user.userid,
        ...
        email: user.email,
        location: user.address || null, // Use address as location
      ...
      return { status: 200, message: 'Login successful', user: sessionUser, token };
    });

    if (loginResult.status === 200) {
      // Send the JWT token back in response
      res.json({ message: loginResult.message, user: loginResult.user, token: loginResult.token });
    } else {
...
app.get('/api/me', authenticateJWT, async (req, res) => {
  ...
  try {
    const [users] = await withConnection((connection) =>
      connection.query('SELECT * FROM nearby_users WHERE userid = ?', [userId])
    );
...
app.post('/api/logout', (req, res) => {
  // No session to destroy. Just inform client to delete token.
  res.json({ message: 'Logged out successfully. Please delete the token on client side.' });
});



// Create a post with media
app.post('/api/createComment', authenticateJWT, upload.array('media', 10), (req, res) => {
...
app.get('/api/getAllPosts', authenticateJWT, (req, res) => {
...
//dELETE ALL POSTS
// Post deletion API
app.delete('/api/deletePost/:postId', authenticateJWT, (req, res) => {
...
app.post('/api/createPost', authenticateJWT, upload.array('media'), (req, res) => {
  const user = req.user;
  if (!user) {
...
// Route
app.put('/api/updatePost/:postId', authenticateJWT, upload.array('media'), (req, res) => {
  const user = req.user;
  if (!user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
//Edit post API
app.put('/api/editPost', authenticateJWT, upload.array('media'), (req, res) => {
  const user = req.user;
...
app.post('/api/emoji', authenticateJWT, async (req, res) => {
...
app.post('/api/reactions', authenticateJWT, (req, res) => {
...
app.put('/api/updateComment', authenticateJWT, authenticateJWT, (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'User is not logged in' });
  }
...
app.post('/api/createReply', authenticateJWT, upload.array('media', 10), (req, res) => {
...
app.get('/api/getComments', authenticateJWT, (req, res) => {
...
//DELETE REPLY
app.delete('/api/deleteReply/:commentId', authenticateJWT, (req, res) => {
...
//Edit reply
app.put('/api/editReply',  authenticateJWT,(req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'User is not logged in' });
  }
...
//Delete comment API
app.delete('/api/deleteComment/:commentId', authenticateJWT, (req, res) => {
  const { commentId } = req.params;
...
app.post('/api/likeReply', authenticateJWT, (req, res) => {
...
app.get('/api/user/:userId/likes-summary', authenticateJWT, async (req, res) => {
...
app.post('/api/getReplyLikes', authenticateJWT, (req, res) => {
...
// Like a comment
app.post('/api/likeComment', authenticateJWT, (req, res) => {
  const { commentId } = req.body;
  const userId = req.user.id; // Get userId safely from session
...
// Unlike a comment
app.post('/api/unlikeComment', authenticateJWT, (req, res) => {
  const { commentId } = req.body;
  const userId = req.user.id;
...
app.post('/api/getCommentLikes', authenticateJWT, (req, res) => {
...
app.get('/api/user/:userId/saves-summary-excluding-own', authenticateJWT, async (req, res) => {
  const { userId } = req.params; // The ID of the profile being viewed
...
//Count all posts for the current user
app.get('/api/user/:userId/posts-summary', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const [postsResult] = await connection.query(
      `SELECT COUNT(*) AS total FROM nearby_posts WHERE userid = ?`,
      [userId]
    );
    const postsCount = postsResult[0].total;

    // Count nearby_alerts
    const [alertsResult] = await connection.query(
      `SELECT COUNT(*) AS total FROM nearby_alerts WHERE userId = ?`,
      [userId]
    );
    const alertsCount = alertsResult[0].total;

    // Count nearby_tips
    const [tipsResult] = await connection.query(
      `SELECT COUNT(*) AS total FROM nearby_tips WHERE user_id = ?`,
      [userId]
    );
    const tipsCount = tipsResult[0].total;

    // Sum all
    const totalPosts = postsCount + alertsCount + tipsCount;

    res.status(200).json({ totalPosts });

  } catch (err) {
    console.error(`Error fetching post summary for user ${userId}:`, err);
    res.status(500).json({ error: 'Failed to fetch post summary.' });
  } finally {
    if (connection) connection.release();
  }
});
...
//GROUPS API
// Create a new group
app.post('/api/createGroup', authenticateJWT, async (req, res) => {
...
app.post('/api/updateGroupDetails', authenticateJWT, async (req, res) => {
...
app.get('/api/grouppostcount/:groupId/postCount', authenticateJWT, async (req, res) => {
...
app.get('/api/getGroups', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
  let connection;
...
//ADD USER TO GROUP
app.post('/api/joinGroup', authenticateJWT,async (req, res) => {
...
//GET GROUPS FOR USER
app.get('/api/getUserGroups', authenticateJWT,async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'User is not logged in' });
  }
...
app.get('/api/groupMemberCount/:groupId', authenticateJWT, async (req, res) => {
...
app.get('/api/getGroup/:groupId', authenticateJWT, async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'User is not logged in' });
  }
...
app.post('/api/checkMembership', authenticateJWT, async (req, res) => {
  const { group_id } = req.body;
...
// Leave a group
app.post('/api/leaveGroup', authenticateJWT, async (req, res) => {
  const { group_id } = req.body;
...
app.post('/api/groupbanner/:groupId/banner', authenticateJWT, upload.single('banner'), async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
// Fetch group banner
app.get('/api/groupbanner/:groupId/banner', authenticateJWT, async (req, res) => {
  const { groupId } = req.params;
  let connection;
...
// Fetch group details
app.get('/api/groupdetails/:groupId', authenticateJWT, async (req, res) => {
  const { groupId } = req.params;
  let connection;
...
app.get('/api/getgroupUserList/:groupId/users', authenticateJWT, async (req, res) => {
  const { groupId } = req.params;
...
app.post('/api/addUserToGroup', authenticateJWT, async (req, res) => {
...
app.get('/api/searchUsers', authenticateJWT, async (req, res) => {
...
// Remove user from group
app.post('/api/removeUserFromGroup', authenticateJWT, async (req, res) => {
...
app.post('/api/demoteGroupAdmin/:groupId/user/:userId',  authenticateJWT, async (req, res) => {
...
app.post('/api/makeGroupadmin/:groupId/user/:userId/makeAdmin', authenticateJWT, async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
app.delete('/api/deleteGroup/:groupId', authenticateJWT, async (req, res) => {
  const { groupId } = req.params;
  const userId = req.user?.id;
...
app.get('/api/user/profile/:userId', authenticateJWT, async (req, res) => {
  const { userId } = req.params;
  let connection;
...
app.post('/api/user/profile-picture', authenticateJWT, uploadProfilePicture.single('profilePicture'), async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
// Upload banner image
app.post('/api/user/banner-image', authenticateJWT, uploadBannerImage.single('bannerImage'), async (req, res) => {
...
app.post('/api/products', authenticateJWT, uploadProductImages.array('images', 5), async (req, res) => {
  const userId = req.user.id;
...
app.get('/api/my-businesses', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/products/all', authenticateJWT, async (req, res) => {
...
app.get('/api/products/user', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
  const limit = parseInt(req.query.limit) || 10;
  const offset = parseInt(req.query.offset) || 0;
  let connection;
...
app.get('/api/editproducts/:id', authenticateJWT, async (req, res) => {
  const productId = req.params.id;
  const userId = req.user?.id;
  let connection;
...
//Messages
app.get("/messages/:roomId", authenticateJWT, async (req, res) => {
...
app.post("/messages/send", authenticateJWT, uploadMessageMedia.array("media"), async (req, res) => {
  const {
    from,
    to,
    roomId,
...
app.get("/api/messages/conversations/direct", authenticateJWT, async (req, res) => {
  const currentUserId = req.user.id;
  let connection;
...
app.get("/api/messages/conversations/products", authenticateJWT, async (req, res) => {
  const currentUserId = req.user?.id;
  let connection;
...
//Message edit
app.post("/messages/edit",  uploadMessageMedia.array("media"), authenticateJWT, async (req, res) => {
...
//bLOCK a user
app.post('/api/block-user', authenticateJWT, async (req, res) => {
...
// server.js (Refactored /api/createQuestion endpoint)

app.post('/api/createQuestion', uploadQuestionImages.array('mediaFiles'), authenticateJWT, async (req, res) => {
    const userId = req.user.id;
    ...
    try {
        ...
        if (io) {
                ...
                if (relevantUsers.length > 0) {
                    ...
                    for (const recipient of relevantUsers) {
                        ...
                        const notificationMetadata = {
                            ...
                            senderProfilePicture: formattedNewQuestion.user.profilePicture // Use formattedNewPost for consistency
                        };
                        ...
                        // 7. Emit real-time notification via Socket.IO to specific user room
                        ...
                        io.to(notificationRoom).emit('question_posted', {
                            ...
                            actorUserId: userId,
                            actorFullname: formattedNewQuestion.user.fullName,
                            actorProfilePicture: formattedNewQuestion.user.profilePicture,
                            action_type: 'question_posted',
                            target_type: 'question',
                            target_id: formattedNewQuestion.id,
            ...
            // --- Emit to the general feed ---
            console.log(`📡 Emitting question_posted to 'nearby_feed' room with postId: ${formattedNewQuestion.id}`);
            io.to('nearby_feed').emit('question_posted', formattedNewQuestion);
            console.log('✅ Emitted question_posted event successfully.');
...
app.put('/api/editquestionpost/:id', uploadQuestionImages.array('media'), authenticateJWT, async (req, res) => {
  const userId = req.user.id;
...
app.post('/api/answer', uploadQuestionAnswerImages.array('media'), authenticateJWT, async (req, res) => {
    const user = req.user;
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
console.log (user);
    ...
    try {
        ...
        // Correctly use 'name' and 'profilePic' from the user object
        const actorFullname = user.name;
        const actorProfilePicture = user.profilePic || null;

        // 2. Notify post owner (if not the commenter)
        ...
        if (postOwnerId && postOwnerId !== user.id) {
            const [notificationResult] = await connection.query(`
                INSERT INTO nearby_notifications (
                    ...
                    parent_id,
                    metadata
                ) VALUES (?, ?, ?, 'answer', 'question', ?, 'post', ?, JSON_OBJECT(
                    'comment_snippet', ?, 'post_id', ?, 'post_type', 'question', 'senderName', ?, 'senderProfilePicture', ?
                ))
            `, [
                postOwnerId,
                createdAt,
            ...
            const payload = {
                ...
                message: `${actorFullname} answered your question.`,
                metadata: {
                    comment_snippet: contentSnippet,
                    post_id: postid,
                    post_type: 'question',
                    senderName: actorFullname,
                    senderProfilePicture: actorProfilePicture,
                }
            };
            if (io) {
                ...
                io.to(`user_notifications:${postOwnerId}`).emit('Youtube_notification', payload);
            }
        }
            ...
            if (
                ...
                const [replyNotificationResult] = await connection.query(`
                    INSERT INTO nearby_notifications (
                        ...
                        parent_id,
                        metadata
                    ) VALUES (?, ?, ?, 'reply', 'answer', ?, 'question', ?, JSON_OBJECT(
                        'comment_snippet', ?, 'post_id', ?, 'post_type', 'question', 'senderName', ?, 'senderProfilePicture', ?
                    ))
                `, [
                    parentOwnerId,
                ...
                const replyPayload = {
                    ...
                    read: false,
                    created_at: createdAt,
                    message: `${actorFullname} replied to your answer.`,
                    metadata: {
                        comment_snippet: contentSnippet,
                        post_id: postid,
                        post_type: 'question',
                ...
                if (io) {
                    ...
                    io.to(`user_notifications:${parentOwnerId}`).emit('Youtube_reply_notification', replyPayload);
                }
            }
        }

        // 4. Handle media upload & send response
        let mediaList = [];
        if (files && files.length > 0) {
            const mediaInserts = files.map(file => [
                answerid,
            ...
            mediaList = mediaInserts.map(([_, url, type]) => ({ mediaUrl: url, mediaType: type }));
        }
        ...
        if (io) {
            io.to(`question_${postid}`).emit('new_answer', {
                ...
                user: {
                    ...
                    username: user.username,
                    profilePicture: actorProfilePicture,
                },
                media: mediaList,
            });
        }
...
app.get('/api/answers/:postid', authenticateJWT, async (req, res) => {
  const { postid } = req.params;
  const currentUserId = req.user?.id;
...
app.post('/api/answer/:answerId/like', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
...
app.delete('/api/questions/:id',  authenticateJWT, async (req, res) => {
...
// server.js (Refactored /api/alerts endpoint)
const NEARBY_ALERT_RADIUS_KM = 5; // 5 kilometers
app.post('/api/alerts', uploadAlertImage.array('mediaFiles', 5), authenticateJWT, async (req, res) => {
const { latitude, longitude, title, description, location } = req.body;
    const authenticatedUserId = req.user ? req.user.id : null;
    const authenticatedUserUsername = req.user ? req.user.username : 'Unknown User';
    const authenticatedUserFullName = req.user ? req.user.name : 'Unknown User';
    const authenticatedUserProfilePic = req.user ? req.user.profilePic : '/default-avatar.png';
...
const insertAlertQuery = `
    INSERT INTO nearby_alerts (userId, alertTypeId, title, description, latitude, longitude, location, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`;
const [insertResult] = await connection.query(insertAlertQuery, [
    ...
    alertLat,
    alertLng,
    location, // <--- Add the location variable here
    createdAt,
]);
...
const alertMetadata = {
    ...
    alertLocationName: location, // <--- Add the human-readable location here
    ...
    location: { lat: newAlertRaw.latitude, lng: newAlertRaw.longitude, name: newAlertRaw.location }, // Add the new location name here
            postType: 'alert',
            user: {
                id: newAlertRaw.user_id_from_db,
                username: newAlertRaw.username,
                fullName: newAlertRaw.fullName,
                profilePicture: newAlertRaw.profile_picture,
            },
            media: mediaArray,
        ...
        if (io) {
            ...
            console.log(`📡 Emitting alert_posted to 'nearby_feed' room with alertId: ${finalNewAlert.id}`);
            io.to('nearby_feed').emit('alert_posted', finalNewAlert); // Emit to the nearby_feed room
            console.log('✅ Emitted alert_posted event successfully.');
        }
...
app.get('/api/alert-types', authenticateJWT, async (req, res) => {
  let connection;
  try {
    connection = await pool.getConnection();
...
app.get('/api/alerts', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    let query = `
            SELECT na.alertId AS id, na.userId, na.alertTypeId, na.title, na.description AS content,
                   na.latitude, na.longitude, na.createdAt AS created_at, na.location,
                   at.name as alert_type_name,
                   u.userid as user_id_from_db, u.username, CONCAT(u.firstname, ' ', u.lastname) AS fullName, u.profile_picture,
                   (
                       SELECT JSON_ARRAYAGG(
    ...
    const alerts = rows.map(row => {
      ...
      return {
        ...
        location: { lat: row.latitude, lng: row.longitude, name: row.location },
        postType: 'alert',
        user: {
...
app.post('/api/alerts/:alertId/comments', uploadAlertImage.array('mediaFiles', 5), authenticateJWT, async (req, res) => {
    const alertId = parseInt(req.params.alertId, 10);
    const userId = req.user.id;
    const user = req.user; // Contains user details from authentication middleware
    ...
    const createdAt = getMySQLDatetime();
    let connection; // Declare connection here to ensure it's in scope for finally block

    // Standardize actor details from req.user
    const actorFullname = user.name;
    const actorProfilePicture = user.profile_picture || null;
    ...
    try {
        ...
        if (alertOwnerId && alertOwnerId !== parsedUserId) {
            ...
            `, [
                ...
                createdAt,
                parsedUserId,
                commentId, // target_id for the notification is the commentId
                alertId,   // parent_id for the notification is the alertId
                comment_snippet,
                alertId
            ]);
            ...
            if (io) {
                ...
                io.to(`user_notifications:${alertOwnerId}`).emit('alert_comment_notification', {
                    id: newlyCreatedNotificationId, // Unique ID for this notification entry
                    recipientId: alertOwnerId,
                    actorId: parsedUserId,
                    actorFullname: actorFullname, // Use standardized variable
                    actorProfilePicture: actorProfilePicture, // Use standardized variable
                    action_type: 'comment',
                    target_type: 'alert', // The type of item being commented on
                    target_id: alertId, // The ID of the alert being commented on
                    parent_type: null, // This is a top-level comment, no parent type
                    parent_id: null,   // No parent ID for a top-level comment
                    created_at: createdAt,
                    message: `${actorFullname} commented on your alert.`, // Frontend can use this or format its own
                    metadata: {
                        senderName: actorFullname,
                        senderProfilePicture: actorProfilePicture,
                        comment_snippet: comment_snippet,
                        alertId: alertId, // Include alertId in metadata for frontend convenience
        ...
        if (parentCommentId) {
            ...
            if (
                ...
                (!alertOwnerId || parentUserId !== alertOwnerId) // Avoid duplicate notifications if parent is also alert owner
            ) {
                const [replyNotificationResult] = await connection.query(`
                    INSERT INTO nearby_notifications (
                        recipient_user_id,
                        ...
                        parent_id,
                        metadata
                    ) VALUES (?, ?, ?, 'reply', 'comment', ?, 'alert', ?, JSON_OBJECT(
                        'comment_snippet', ?,
                        'post_id', ?,
                        'post_type', 'alert'
                    ))
                `, [
                    ...
                    createdAt,
                    parsedUserId,
                    commentId,       // target_id for the notification is the new commentId
                    parentCommentId, // parent_id for the notification is the parentCommentId
                    comment_snippet,
                    alertId          // post_id in metadata refers to the original alert
                ]);
                const newlyCreatedReplyNotificationId = replyNotificationResult.insertId;
                ...
                if (io) {
                    ...
                    // Emit real-time reply notification
                    io.to(`user_notifications:${parentUserId}`).emit('alert_reply_notification', {
                        id: newlyCreatedReplyNotificationId, // Unique ID for this notification entry
                        recipientId: parentUserId,
                        actorId: parsedUserId,
                        actorFullname: actorFullname, // Use standardized variable
                        actorProfilePicture: actorProfilePicture, // Use standardized variable
                        action_type: 'reply',
                        target_type: 'comment', // The type of item being replied to
                        target_id: commentId,   // The ID of the comment being replied to
                        parent_type: 'alert',   // The type of the original parent (alert)
                        parent_id: alertId,     // The ID of the original parent (alert)
                        created_at: createdAt,
                        message: `${actorFullname} replied to your comment on an alert.`,
                        metadata: {
                            senderName: actorFullname,
                            senderProfilePicture: actorProfilePicture,
                            comment_snippet: comment_snippet,
                            alertId: alertId, // Include alertId in metadata for frontend convenience
                            commentId: commentId, // Include commentId in metadata
                            parentCommentId: parentCommentId // Include parentCommentId in metadata
                        },
                    });
                }
            }
        ...
        // 📣 Emit to everyone watching the alert (general feed update)
        if (io) {
            io.to(`alert_${alertId}`).emit('new_alert_comment', {
                ...
                userId: parsedUserId,
                createdAt,
                user: { // Include user details for the general feed
                    fullName: actorFullname,
                    username: user.username,
                    profilePicture: actorProfilePicture,
                }
            ...
            console.warn("Socket.IO instance not found on req.app.get('io') for general alert feed emission.");
        }
...
//Get alert comments:
app.get('/api/alerts/:alertId/comments', authenticateJWT, async (req, res) => {
  const alertId = parseInt(req.params.alertId, 10);
  const currentUserId = req.user?.id;
...
app.post('/api/tip_comments', uploadTipMedia.array('mediaFiles', 10), authenticateJWT, async (req, res) => {
    const userId = req.user.id;
    ...
    try {
        ...
        // --- Fetch actor's full user details for consistent notification data ---
        const [actorUserRows] = await connection.query(
            'SELECT username, CONCAT(firstname, " ", lastname) AS fullname, profile_picture FROM nearby_users WHERE userid = ?',
            [userId]
        );
        ...
        const actorFullname = actorUser.fullname;
        const actorProfilePicture = actorUser.profile_picture;
        // --- END User Details Fetch ---
        ...
        if (postOwnerId && postOwnerId !== userId) {
            try {
                const [notificationInsertResult] = await connection.query(`
                    INSERT INTO nearby_notifications (
                        recipient_user_id,
                        actor_user_id,
                        action_type,
                ...
                `, [
                    postOwnerId,
                    userId,
                    post_type,  // target_type: the type of post being commented on
                    commentId,  // target_id: the ID of the new comment
                    post_type,  // parent_type: the type of the original post
                    post_id,    // parent_id: the ID of the original post
                    createdAt,
                    commentSnippet,
                    post_id,
                    post_type
                ]);
                const newlyCreatedNotificationId = notificationInsertResult.insertId;
                ...
                if (io) {
                    ...
                    // Standardized payload for comment notification
                    io.to(`user_notifications:${postOwnerId}`).emit('tip_comment_notification', {
                        id: newlyCreatedNotificationId, // Use the actual notification ID
                        recipientId: postOwnerId,
                        actorId: userId,
                        actorFullname: actorFullname, // Use fetched actor's full name
                        actorProfilePicture: actorProfilePicture, // Use fetched actor's profile picture
                        action_type: 'comment',
                        target_type: post_type, // Type of the parent post
                        target_id: post_id,     // ID of the parent post
                        parent_type: null,      // No parent for a top-level comment notification
                        parent_id: null,        // No parent ID for a top-level comment notification
                        read: false,
                        is_read: false,
                        created_at: createdAt,
                        message: `${actorFullname} commented on your ${post_type}.`, // Dynamic message
                        metadata: {
                ...
                console.error('❌ Notification insert or emit failed for comment:', err);
            }
        }
        ...
        if (parentIdValue) {
            ...
            // Avoid notifying if parent owner is self or the original post owner (if already notified)
            if (
                parentOwnerId &&
                parentOwnerId !== userId &&
                parentOwnerId !== postOwnerId // Ensure no duplicate notification if parent is also post owner
            ) {
                const [replyNotificationInsertResult] = await connection.query(`
                    INSERT INTO nearby_notifications (
                        recipient_user_id,
                        created_at,
                        actor_user_id,
                        ...
                        'comment_snippet', ?, 'post_id', ?, 'post_type', ?, 'parent_comment_id', ?
                    ))
                `, [
                    parentOwnerId,
                    createdAt,
                    userId,
                    'reply',        // action_type for reply
                    'comment',      // target_type: the type of item being replied to (a comment)
                    commentId,      // target_id: the ID of the new comment (the reply itself)
                    post_type,      // parent_type: the type of the original post
                    post_id,        // parent_id: the ID of the original post
                    commentSnippet,
                    post_id,
                ...
                const newlyCreatedReplyNotificationId = replyNotificationInsertResult.insertId;
                ...
                if (io) {
                    ...
                    // Standardized payload for reply notification
                    io.to(`user_notifications:${parentOwnerId}`).emit('tip_reply_notification', {
                        id: newlyCreatedReplyNotificationId, // Use the actual notification ID
                        recipientId: parentOwnerId,
                        actorId: userId,
                        actorFullname: actorFullname, // Use fetched actor's full name
                        actorProfilePicture: actorProfilePicture, // Use fetched actor's profile picture
                        action_type: 'reply',
                        target_type: 'comment', // The reply is on a comment
                        target_id: commentId,   // The ID of the new comment (the reply)
                        parent_type: post_type, // The original post's type
                        parent_id: post_id,     // The original post's ID
                        read: false,
                        is_read: false,
                        created_at: createdAt,
                        message: `${actorFullname} replied to your comment on a ${post_type}.`, // Dynamic message
                        metadata: {
                            comment_snippet: commentSnippet,
                            post_id: post_id,
                            post_type: post_type,
        ...
        // 5. Emit to post room (for general feed updates, not specific notifications)
        if (io) {
            ...
            console.warn("Socket.IO instance not found on req.app.get('io') for general feed emission.");
        }
...
app.delete('/api/alerts/:id', authenticateJWT, async (req, res) => {
  const alertId = parseInt(req.params.id, 10);
  const userId = req.user?.id;
...
// server.js (Refactored /api/nearby_tips endpoint)

app.post('/api/nearby_tips', uploadTipMedia.array('mediaFiles', 10), authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
        ...
        const fetchNewTipQuery = `
            SELECT
              ...
              (
                ...
                FROM nearby_tip_media tm
                WHERE tm.tip_id = t.id
              ) AS media,
              ns.id AS savedPostId, CASE WHEN ns.id IS NOT NULL THEN TRUE ELSE FALSE END AS isSaved,
              nl.id AS likedPostId, CASE WHEN nl.id IS NOT NULL THEN TRUE ELSE FALSE END AS isLiked,
        ...
        const io = req.app.get('io');
        if (io) {
            console.log(`📡 Emitting tip_posted to 'nearby_feed' room with tipId: ${formattedNewTip.id}`);
            io.to('nearby_feed').emit('tip_posted', formattedNewTip); // Emit to the nearby_feed room
            console.log('✅ Emitted tip_posted event successfully.');
        } else {
            console.warn("Socket.IO instance not found on req.app.get('io')");
        }
...
app.put('/api/edittip/:tipId', uploadTipMedia.array('mediaFiles', 10), authenticateJWT, async (req, res) => {
  const tipId = req.params.tipId;
  const userId = req.user?.id; // Assuming userId from JWT for authorization
...
app.get('/api/nearby_tips', authenticateJWT, async (req, res) => {
  const currentUserId = req.user.id;
...
app.get('/api/tip_comments/:post_type/:post_id', authenticateJWT, async (req, res) => {
...
app.delete('/api/tips/:id', authenticateJWT, async (req, res) => {
  const tipId = parseInt(req.params.id, 10);
  const userId = req.user?.id; // Assuming user ID is from JWT
...
app.post('/api/saved_posts', authenticateJWT, async (req, res) => {
    const { post_id, post_type } = req.body;
    const userId = req.user?.id; // This is the ID of the user performing the save
    ...
    try {
        ...
        // 1️⃣ check if already saved (your existing logic)
        ...
        if (existing.length > 0) {
            return res.status(409).json({
                ...
                id: existing[0].id // IMPORTANT: Return the existing saved_post ID for client-side updates
            });
        }

        // 2️⃣ insert save record (your existing logic)
        const [result] = await connection.query(
            'INSERT INTO nearby_saved_posts (user_id, post_id, post_type) VALUES (?, ?, ?)',
            [userId, post_id, post_type]
        );

        /* --- ADD THIS SECTION TO FETCH ACTOR DETAILS --- */
        // Fetch the details of the user who saved the post (the actor)
        const [actorRows] = await connection.query(
            `SELECT CONCAT(firstname, ' ', lastname) AS fullname, profile_picture FROM nearby_users WHERE userid = ?`, // Assuming 'nearby_users' is your user table
            [userId]
        );
        const actor = actorRows?.[0]; // Get the first row, which should be the user

        if (!actor) {
            console.warn(`Actor with userId ${userId} not found after save operation.`);
            // You might choose to continue without sending notification or rollback
            // For now, we'll just log and assume notifications might be incomplete.
        }
        /* --- END OF ADDED SECTION --- */

        // 3️⃣ determine table and snippet column (your existing logic)
        const meta = {
        ...
        // 4️⃣ fetch post owner + snippet (your existing logic)
        ...
        const createdAt = getMySQLDatetime(); // Ensure getMySQLDatetime is defined or imported

        // 5️⃣ notify post owner (if not self) (your existing logic)
        if (recipientId && recipientId !== userId && actor) { // Add 'actor' check here
            ...
            // 6️⃣ emit socket event (your existing logic, now using 'actor' details)
            io.to(`user_${recipientId}`).emit('post_save_notification', {
                id: notificationId,
                recipientUserId: recipientId,
                actorUserId: userId,
                actorFullname: actor.fullName,      // Use actor.fullName
                actorProfilePicture: actor.profilePicture, // Use actor.profilePicture
                action_type: 'save',
        ...
        // Return the insertId so the client can use it for subsequent unsave operations
...
app.delete('/api/saved_posts/:id', authenticateJWT, async (req, res) => {
...
app.get('/api/saved_posts/me', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
...
// To like a post
app.post('/api/like', authenticateJWT, async (req, res) => {
    const { post_id: postId, post_type: postType } = req.body;
    const userId = req.user?.id; // This is the ID of the user who is performing the like
    ...
    try {
        ...
        /* --- ADD THIS SECTION TO FETCH ACTOR DETAILS --- */
        // Fetch the details of the user who liked the post (the actor)
        const [actorRows] = await connection.query(
            `SELECT CONCAT(firstname, ' ', lastname) AS fullName, profile_picture FROM nearby_users WHERE userid = ?`, // Assuming 'nearby_users' is your user table
            [userId]
        );
        ...
        if (!actor) {
            // This case should ideally not happen if userId is valid, but good for robustness
            console.warn(`Actor with userId ${userId} not found after like operation.`);
            // You might choose to continue without sending notification or rollback
            // For now, we'll just log and assume notifications might be incomplete.
        }
        /* --- END OF ADDED SECTION --- */
        ...
        if (recipientId && recipientId !== userId && actor) { // Ensure actor details are available
            const createdAt = getMySQLDatetime();
...
app.delete('/api/like/:postId/:postType', authenticateJWT, async (req, res) => {
...
app.delete('/api/comments/:commentId/like', authenticateJWT, async (req, res) => {
  const { commentId } = req.params;
...
app.post('/api/replies/:replyId/like', authenticateJWT, async (req, res) => {
...
app.delete('/api/replies/:replyId/like', authenticateJWT, async (req, res) => { // Added 'async'
  const { replyId } = req.params;
...
//
app.put('/api/tips/:postId/comments/:commentId',  uploadTipMedia.array('mediaFiles', 10), authenticateJWT, async (req, res) => {
  ...
  try {
    await withConnection(async (conn) => {
      ...
      await conn.query(
        ...
        [safeContent, parentCommentId, editedAt, commentId, postId, req.user.id]
      );
...
app.put('/api/questions/:postId/answers/:commentId', uploadAnswer.array('files'), authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
app.delete('/api/questions/:postId/answers/:answerId', authenticateJWT, async (req, res) => {
  const postId = parseInt(req.params.postId, 10);
...
app.delete('/api/tips/:postId/comments/:commentId', authenticateJWT, async (req, res) => {
  const postId = parseInt(req.params.postId, 10);
  const commentId = parseInt(req.params.commentId, 10);
...
app.delete('/api/alerts/:alertId/comments/:commentId', authenticateJWT, async (req, res) => {
  const alertId = parseInt(req.params.alertId, 10);
  const commentId = parseInt(req.params.commentId, 10);
...
//DDiscussion Post type

app.post('/api/createDiscussionPost', uploadDiscussionMedia.array('mediaFiles'), authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
   const [result] = await connection.query(
  'INSERT INTO nearby_posts (userid, content, created_at, type) VALUES (?, ?, ?, "discussion")',
  [userId, content, createdAt]
);
...
   if (io) {
  console.log(`📡 Emitting new_discussion_post to 'nearby_feed' room with postId: ${formattedNewPost.id}`);
  io.to('nearby_feed').emit('new_discussion_post', formattedNewPost);
  console.log('✅ Emitted new_discussion_post event successfully.');
} else {
...
app.get('/api/discussions', authenticateJWT, async (req, res) => {
...
//SIngle discussion post data

app.get('/api/discussions/:id', authenticateJWT, async (req, res) => {
    const currentUserId = req.user?.id || 0;
    ...
    try {
        await withConnection(async (conn) => {
            // Fetch the specific post with user data, saved and liked status, like count
            const getPostQuery = `
                SELECT 
                    p.postid AS postId,
                    p.content,
                ...
                LEFT JOIN nearby_likes nl 
                    ...
                    AND nl.post_type = 'discussion' 
                    AND nl.user_id = ?
                WHERE p.type = 'discussion' AND p.postid = ? -- Filter by specific post ID
            `;

            const [posts] = await conn.query(getPostQuery, [currentUserId, currentUserId, postId]);

            if (posts.length === 0) {
                return res.status(404).json({ error: 'Discussion post not found' });
            }

            const post = posts[0]; // We expect only one post

            // Fetch media for the single post
            const getMediaQuery = `
                SELECT postid, media_url, media_type
                FROM nearby_post_media
                WHERE postid = ?
            ...
            // Format the post with media and user info
            const formattedPost = {
                postId: post.postId,
                content: post.content,
                createdAt: post.createdAt,
            ...
            res.json(formattedPost); // Return the single formatted post
        });
    } catch (err) {
        console.error(`Error fetching discussion post with ID ${postId}:`, err);
...
app.put('/api/editdiscussionpost/:id', uploadDiscussionMedia.array('media'), authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
app.delete('/api/deletediscussion/:id', authenticateJWT, async (req, res) => {
  const user = req.user;
  const discussionId = req.params.id;
...
app.post('/api/discussion/comments', uploadDiscussionMedia.array('media'), authenticateJWT, async (req, res) => {
    const user = req.user;
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
        ...
        await withConnection(async (conn) => {
            ...
            if (postRows.length > 0) {
                ...
                if (postOwnerId && postOwnerId !== userId) {
                    ...
                    // Standardize the payload to match the frontend
                    io.to(`user_notifications:${postOwnerId}`).emit('discussion_comment_notification', {
                        id: newlyCreatedNotificationId, // Add the notification ID
                        recipientId: postOwnerId,
                        actorId: userId,
                        actorFullname: actorFullname,
                        actorProfilePicture: actorProfilePicture,
                        ...
                        target_id: postId,
                        created_at: createdAt,
                        message: `${actorFullname} commented on your discussion.`,
                        metadata: {
            ...
            if (parentCommentId) {
                ...
                if (parentRows.length > 0) {
                    ...
                    if (
                        ...
                        const [replyNotificationResult] = await conn.query(`
                          INSERT INTO nearby_notifications (
                            recipient_user_id, created_at, actor_user_id, action_type,
                            target_type, target_id, parent_type, parent_id, metadata
                        ...
                        io.to(`user_notifications:${parentUserId}`).emit('discussion_reply_notification', {
                            id: newlyCreatedReplyNotificationId, // Add the notification ID
                            recipientId: parentUserId,
                            actorId: userId,
                            actorFullname: actorFullname,
                            actorProfilePicture: actorProfilePicture,
                            ...
                            created_at: createdAt,
                            message: `${actorFullname} replied to your comment on a discussion.`,
                            metadata: {
                                comment_snippet: snippet,
                                post_id: postId,
                                post_type: 'discussion',
...
app.delete('/api/discussion/:postId/comments/:commentId', authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/discussion/comments/:postid', authenticateJWT, async (req, res) => {
  const postId = req.params.postid;
...
/////   Update  comment /////
app.put('/api/discussion/comments/:commentid', uploadDiscussionMedia.array('mediaFiles'), authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
////////////////////////////////////////////////////
app.post('/api/reportpost', authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
//Get Group posts
app.get('/api/groups/:groupId/posts', authenticateJWT, async (req, res) => {
...
app.get('/api/groups/:postId/comments', authenticateJWT, async (req, res) => {
...
app.get('/api/user/:userId/comments-summary', authenticateJWT, async (req, res) => {
...
app.get('/api/alerts/:alertId', authenticateJWT, async (req, res) => {
...
app.get('/api/nearby_tips/:tipId', authenticateJWT, async (req, res) => {
...
app.get('/api/discussions/:discussionId', authenticateJWT, async (req, res) => {
...
app.post('/api/events/:eventId/rsvp', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
...
app.put('/api/events/:id', uploadEventMedia.array('mediaFiles', 5), authenticateJWT, async (req, res) => {
...
app.delete('/api/events/:eventId/rsvp', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
  const eventId = req.params.eventId;
...
app.get('/api/events/attending', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/events/mine', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/events/:eventId/attendees', authenticateJWT,async (req, res) => {
  const eventId = req.params.eventId;
...
app.delete('/api/events/:eventId', authenticateJWT, async (req, res) => {
  const userId = req.user?.id;
  const eventId = req.params.eventId;
...
app.post('/api/notifications/:id/mark-read',authenticateJWT, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
...
app.post('/api/businesses',authenticateJWT, uploadBusinessMedia.fields([
...
app.put('/api/businesses/:id',authenticateJWT, uploadBusinessMedia.fields([
  { name: 'logo', maxCount: 1 },
  { name: 'banner', maxCount: 1 },
]), async (req, res) => {
  if (!req.user) return res.status(401).json({ error: 'User not logged in' });
...
app.get('/api/business_categories',authenticateJWT, async (req, res) => {
  let connection;
  try {
    connection = await pool.getConnection();
...
app.get('/api/businesses',authenticateJWT, async (req, res) => {
  let connection;
  try {
    connection = await pool.getConnection();
...
app.get('/api/businesses/:id',authenticateJWT, async (req, res) => {
  const businessId = parseInt(req.params.id);
  if (isNaN(businessId)) {
...
app.put('/api/businesses/:id/hours',authenticateJWT, async (req, res) => {
  const { id } = req.params;
...
app.put('/api/businesses/:id/services',authenticateJWT, async (req, res) => {
...
//Business events:
app.post('/api/business/:id/events',authenticateJWT, async (req, res) => {
  const businessId = parseInt(req.params.id);
...
app.post('/api/businesses/:id/events', authenticateJWT, uploadBusinessMedia.array('media', 5), async (req, res) => {
  const businessId = req.params.id;
...
app.put('/api/businesses/:id/events/:eventId', authenticateJWT, uploadBusinessMedia.array('media', 5), async (req, res) => {
  const businessId = req.params.id;
  const eventId = req.params.eventId;
...
app.get('/api/businesses/:id/events', authenticateJWT, async (req, res) => {
  const businessId = req.params.id;
  let connection;
...
app.delete('/api/businesses/:businessId/events/:eventId', authenticateJWT, async (req, res) => {
...
app.get('/api/events/by-business', authenticateJWT, async (req, res) => {
  const sessionUserId = req.user.id;
  let connection;
...
app.post('/api/businessevents/:eventId/attend', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
...
app.delete('/api/businessevents/:eventId/attend', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
  const { eventId } = req.params;
...
app.get('/api/businessevents/:eventId/attendees', authenticateJWT, async (req, res) => {
...
app.get('/api/businessevents/attending', authenticateJWT, async (req, res) => {
  const userId = req.user.id;
...
app.get('/api/businesses/:businessId/products', authenticateJWT, async (req, res) => {
...
////MESSAGING LOGIC
app.post('/api/businessmessages', authenticateJWT, uploadBusinessMedia.array('media'), async (req, res) => {
  const {
...
app.get('/api/businessmessages/:roomId', authenticateJWT, async (req, res) => {
...
app.get('/api/businesses/:businessId/conversations', authenticateJWT, async (req, res) => {
  let connection;
  try {
...
app.get('/api/productmessages/:roomId', authenticateJWT, async (req, res) => {
  let connection;
  try {
    const { roomId } = req.params;
...
app.post('/api/upload/message-media', authenticateJWT, uploadMessageMedia.single('mediaFile'), (req, res) => {
...
app.get("/api/messages/business-conversations", authenticateJWT, async (req, res) => {
  const currentUserId = req.user.id;
  let connection;
...
app.get("/api/messages/product-conversations", authenticateJWT, async (req, res) => {
  const currentUserId = req.user.id;
  let connection;
